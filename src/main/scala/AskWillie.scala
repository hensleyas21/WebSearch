...
@main def main(): Unit = {
        ...
        val rankedPages: List[RankedWebPage] = PageRank.pagerank(pages).map((id, weight) => new RankedWebPage(pages(id), weight)).toList // call PageRank.???? here
        ...
        while {
          ...
          val searchedPages: List[SearchedWebPage] = PageSearch.tfidf(rankedPages, terms).zip(rankedPages).map((textmatch, rankedPage) => new SearchedWebPage(rankedPage,textmatch)) // call PageSearch.???? here
